...
interface ThreeModule {
  ...
  BufferGeometry: new () => { setAttribute: (name: string, attr: object) => void };
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      const palette = [
        [0.388, 0.784, 1.0], [0.655, 0.545, 0.98],
        [0.957, 0.443, 0.714], [0.984, 0.573, 0.188], [0.302, 0.871, 0.502],
      ];
      for (let i = 0; i < N; i++) {
      ...
      const particleMat = new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          void main() {
            ...
            float wave = sin(p.x*0.3+time*0.5)*0.3+cos(p.y*0.25+time*0.4)*0.3; p.z += wave;
            vec2 dm = vec2(p.x,p.y)-mouse*12.0; float dist=length(dm);
            float push=max(0.0,3.0-dist)*0.4; p.xy+=normalize(dm+0.001)*push;
      ...
      const ringMatBase = new THREE.ShaderMaterial({
        ...
        vertexShader: `varying float vPos; void main(){vPos=position.x;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,
        fragmentShader: `varying float vPos; uniform float time; void main(){float t=mod(vPos*0.14+time*0.5,1.0);float alpha=pow(sin(t*3.14159),2.0)*0.7;vec3 c1=vec3(0.388,0.784,1.0);vec3 c2=vec3(0.655,0.545,0.98);gl_FragColor=vec4(mix(c1,c2,t),alpha);}`,
      ...
      for (let i = 0; i < 5; i++) {
        ...
        ring.userData.axis  = new THREE.Vector3(Math.random()-0.5, Math.random()-0.5, Math.random()-0.5).normalize();
        scene.add(ring);
      }
      ...
      const sphereMat = new THREE.ShaderMaterial({
        uniforms: { time: { value: 0 } },
        vertexShader: `varying vec3 vNormal;varying vec3 vPos;uniform float time;void main(){vNormal=normalize(normalMatrix*normal);vPos=position;float dist=0.12*sin(5.0*position.y+time*1.5)*cos(5.0*position.x+time);vec3 newPos=position+normal*dist;gl_Position=projectionMatrix*modelViewMatrix*vec4(newPos,1.0);}`,
        fragmentShader: `varying vec3 vNormal;varying vec3 vPos;uniform float time;void main(){float rim=1.0-max(dot(vNormal,vec3(0,0,1)),0.0);rim=pow(rim,2.5);vec3 c1=vec3(0.388,0.784,1.0);vec3 c2=vec3(0.655,0.545,0.98);vec3 c3=vec3(0.957,0.443,0.714);float t=sin(time*0.5+vPos.y*0.5)*0.5+0.5;float t2=sin(time*0.3+vPos.x*0.5)*0.5+0.5;vec3 col=mix(mix(c1,c2,t),c3,t2*0.5);gl_FragColor=vec4(col*rim,rim*0.6);}`,
        transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.FrontSide,
      } as object);
      const sphere = new THREE.Mesh(new THREE.SphereGeometry(4.5, 64, 64), sphereMat);
      scene.add(sphere);

      const wire = new THREE.Mesh(
        new THREE.IcosahedronGeometry(5, 2),
        new THREE.MeshBasicMaterial({ color: 0x3399ff, wireframe: true, transparent: true, opacity: 0.06 })
      );
      scene.add(wire);
      ...
      const animate = () => {
        ...
        sphere.rotation.y = elapsed * 0.1; sphere.rotation.x = mouse.y * 0.1;
        wire.rotation.y   = elapsed * 0.05; wire.rotation.x   = elapsed * 0.03;
  ...
  return (
    <>
      <style>{`
        ...
        /* Quote button = styled like a button not a link */
        .hero-btn-quote {
        ...
        .hero-btn-quote:hover {
          ...
          box-shadow: 0 0 50px rgba(99,200,255,.5), 0 0 80px rgba(167,139,250,.3) !important;
        }
      ...
      >
        ...
        <div aria-hidden style={{
          ...
          backgroundImage: "linear-gradient(rgba(99,200,255,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(99,200,255,0.04) 1px,transparent 1px)",
        ...
        <div id="hero-cursor-glow" aria-hidden style={{
          ...
          background: "radial-gradient(circle,rgba(99,200,255,.08) 0%,transparent 70%)",
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div className="hero-buttons" style={{
              ...
              {/* Iniciar Projecto → opens QuoteModal */}
              <button
            ...
            <div className="hero-stats hero-stats-wrap" style={{
              ...
              {[
                ...
                <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 40 }}>
                  {i > 0 && <div style={{ width: 1, height: 32, background: "rgba(255,255,255,.07)", flexShrink: 0 }} />}
      ...
      {/* ── Quote Modal (rendered outside section so it overlays everything) ── */}
